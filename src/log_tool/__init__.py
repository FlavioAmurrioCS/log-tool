...
class LogTool:
    ...
    stream: FunctionalStream[LogLine]  # pyright: ignore[reportGeneralTypeIssues]
    processed_stream: FunctionalStream[LogLine]  # pyright: ignore[reportGeneralTypeIssues]
    ...
    def reload_config(self) -> None:
        ...
        if self.log_pattern != log_pattern:
            ...
            self.stream = (  # pyright: ignore[reportGeneralTypeIssues]
                seq(self.log_files)
    ...
    def __process__(self) -> FunctionalStream[LogLine]:  # pyright: ignore[reportGeneralTypeIssues]
        ...
        msg_patterns = self.log_tool_config.get("MSG") or []
        if msg_patterns:
    ...
    def m_print(self, *, original_stream: bool = False) -> None:
        stream = self.stream if original_stream else self.processed_stream
        print_format = self.log_tool_config.get("print_format")
        func = (
            (lambda log_line: log_line.line)
            if print_format is None
            else (lambda log_line: print_format.format_map(log_line.parts))
        )
...
def pretty_search(
    config_yaml: str,
    files: list[str] = typer.Option([], "--file", "-f", help="Files to search."),  # noqa: B008
...
def search(
    ...
    files: list[str] = typer.Option(["/dev/stdin"], "--file", "-f", help="Files to search."),  # noqa: B008
    ignore_patterns: list[str] = typer.Option([], "-v", help="Specify ignore pattern"),  # noqa: B008
) -> None:
    """Search and color text files."""
    files = [("/dev/stdin" if file == "-" else file) for file in files]
    stream: FunctionalStream[str] = seq(files).flat_map(
