...
class LogTool():
    ...
    def reload_config(self):
        ...
        if self.log_pattern != log_pattern:
            ...
            def line_to_logline(line) -> LogLine | None:
    ...
    def __process__(self) -> FunctionalStream[LogLine]:
        ...
        loglevels = set(self.log_tool_config.get('LOGLEVEL', []))
        if loglevels:
            predicates.append(lambda x: x.parts['LOGLEVEL'] in loglevels)

        classes = set(self.log_tool_config.get('CLASS', []))
        if classes:
            predicates.append(lambda x: x.parts['CLASS'] in classes)

        msg_patterns = self.log_tool_config.get('MSG', [])
        if msg_patterns:
...
def search(
    ...
    patterns_and_colors: list[tuple[re.Pattern, str]] = [
